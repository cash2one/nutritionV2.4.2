...
public class CheckIdCardNumber {
	...
	private final String[] errMsg = new String[] { "身份证完全正确！", "身份证为空！", "身份证长度不正确！",
			"身份证有非法字符！", "身份证中出生日期不合法！", "身份证校验位错误！" };
	 ...
	 * @param idCardNum
	 */
	public CheckIdCardNumber(String idCardNum) {
		// super();
	...
	public String to15() {
		...
		if (this.is15())
			...
			return this.idCardNum.substring(0, 6)
					+ this.idCardNum.substring(8, 17);
	}
	...
	public String to18() {
		...
		if (this.is18())
			...
			return this.idCardNum.substring(0, 6) + "19"
					+ this.idCardNum.substring(6) + this.getCheckBit();
	}
	...
	public static String toNewIdCard(String tempStr) {
		if (tempStr.length() == 18)
			...
			return tempStr.substring(0, 6) + "19" + tempStr.substring(6)
					+ getCheckBit(tempStr);
	}
	...
	public int isCorrect() {
		if (this.isEmpty()) {
			this.error = CheckIdCardNumber.IS_EMPTY;
			return this.error;
		}
		if (!this.isLenCorrect()) {
			this.error = CheckIdCardNumber.LEN_ERROR;
			return this.error;
		}
		if (!this.isCharCorrect()) {
			this.error = CheckIdCardNumber.CHAR_ERROR;
			return this.error;
		}
		if (!this.isDateCorrect()) {
			this.error = CheckIdCardNumber.DATE_ERROR;
			return this.error;
		}
		/*if (this.is18()) {
			if (!this.getCheck().equals(this.getCheckBit())) {
				this.error = CheckIdCardNumber.CHECK_BIT_ERROR;
				return this.error;
			}
		}*/
		return 0;
	...
	private String getCheckBit() {
		...
		if (this.is18())
			...
			temp = this.idCardNum.substring(0, 6) + "19"
					+ this.idCardNum.substring(6);
